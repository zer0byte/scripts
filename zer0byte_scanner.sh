...
[ ! -d "results/nmap" ] && mkdir -p results/nmap
[ ! -d "results/nikto" ] && mkdir -p results/nikto
[ ! -d "results/dirb" ] && mkdir -p results/dirb
[ ! -d "results/eyewitness" ] && mkdir -p results/eyewitness
...
start_dirb_scans() {
  for target in $targets; do
    ...
    start_screen_session "dirb_http_$target" "dirb $http_target /root/common.txt -f -o results/dirb/dirb_http_$target.txt"
    start_screen_session "dirb_https_$target" "dirb $https_target /root/common.txt -f -o results/dirb/dirb_https_$target.txt"
  done
}
...
start_nikto_scans() {
  for target in $targets; do
    http_target="http://$target"
    https_target="https://$target"
    start_screen_session "nikto_http_$target" "/root/Tools/nikto/program/nikto.pl -h $http_target -404code 301 -C all -output results/nikto/nikto_http_$target.txt"
    start_screen_session "nikto_https_$target" "/root/Tools/nikto/program/nikto.pl -h $https_target -404code 301 -C all -output results/nikto/nikto_https_$target.txt"
  done
}
...
start_nmap_scans() {
  for target in $targets; do
    start_screen_session "nmap_$target" "nmap $target -sV --open -oA results/nmap/nmap_$target"
  done
}
